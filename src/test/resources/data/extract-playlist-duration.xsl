...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.example.com/xslt-functions" exclude-result-prefixes="fn" >
	<xsl:template match="*">
		<xsl:copy>
			<xsl:apply-templates/>
		</xsl:copy>
	...
	<xsl:function name="fn:seconds-to-time">
		...
		<xsl:number format="01" value="$seconds idiv 60"/>
		<xsl:text>:</xsl:text>
		<xsl:number format="01" value="$seconds mod 60"/>
	</xsl:function>
